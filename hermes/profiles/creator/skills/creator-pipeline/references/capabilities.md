...
| a UI task walkthrough authored from reference screenshots/design/text, with visible state changes and action-following camera | video-creator: create-tour | free, <=60 seconds; task-local HTML/CSS/GSAP source/preview/MP4, always kind="work"; free-text intro/outro, default ON; no URL capture or browser automation; optional finished audio-creator WAV/words.json |
...
7. Bounded UI task walkthroughs select create-tour before the legacy table;
   ...
   Route by authorship method as well as container. A model-generated MP4 is
    `generate-clip` within its short-shot contract; a requested legacy
     backend stays `creator-generated-video`. Broader seekable HTML motion is

...
direction for bounded task-local CSS, not permission to select a nearby named
look. Do not offer generation to satisfy a CSS-only style change.
...
stay on tile 1. Preview gaps are simulation choices, never known platform UI.
...
Unlike the generic defaults below, paid Card generation always needs explicit
user budget approval in this work conversation before any call. Ask in the
same round as reference-upload consent and backend aspect limitations; never
promise unsupported panorama ratios or silently spend extra per tile.
...
now has a served route (`create-explainer-video`, below); requested work
still outside every served contract (multi-shot montage, ComfyUI,
...
learning_goal — never a UI task walkthrough (`create-tour`), an
advertisement (`create-ad`), or a model-generated music-video-style piece
(`generate-music-video`). Always `specialist_call(kind="work")` even
...
sequenced the same way as any other composite request (`build.md`).

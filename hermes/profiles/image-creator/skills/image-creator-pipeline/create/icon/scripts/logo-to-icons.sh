...
magick "$MASTER" -resize 180x180 -background "$BG" -flatten -alpha off "$OUTDIR/apple-icon.png"
...
# One machine-readable line: every file with its measured size, so the QA
# dimension check reads the script's own measurement rather than guessing.
FILES=""
for f in favicon.ico apple-icon.png icon-maskable-512.png; do
  FILES="$FILES $f=$(magick identify -format '%wx%h ' "$OUTDIR/$f" | awk '{print $1}')"
done
for s in "${SZ[@]}"; do
  FILES="$FILES icon-${s}.png=$(magick identify -format '%wx%h' "$OUTDIR/icon-${s}.png")"
done
MASK_COVER="$(magick "$OUTDIR/icon-maskable-512.png" -alpha off -fuzz 2% -trim -format '%w' info: 2>/dev/null || echo '?')"
echo "RESULT: out=$OUTDIR bg=$BG color=${COLOR:-none} sizes=$SIZES mark_extent_in_maskable=${MASK_COVER}px_of_512${FILES}"

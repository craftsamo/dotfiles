...
2. Run `git log -n 5` to identify the language to use for commits and consider which language is appropriate.

3. Run `git status` to see the files staged for commit.

4. Use `git diff --cached` to review the staged changes.

5. Commit the changes with `git commit` based on the gathered information.
...
6. Ensure the commit format is valid and adheres to the rules.
]]
...
3. Use `git log {{base_branch}}..{{current_branch}} --reverse --pretty="%an -> %s"` to review the commit messages being aggregated.

4. Run `git diff {{base_branch}}` to examine the differences from the SquashMerge target and understand the changes.

5. Identify relevant pull requests based on the collected information.
  - Ensure the pull request is currently open.

6. At this stage, carefully consider the commit content.
  - Focus on the consolidated message, keeping in mind that multiple commits will be merged into a single commit.

7. Ensure the final formatting adheres to the following structure:
